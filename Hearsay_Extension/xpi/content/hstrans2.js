			...
			onInputStreamReady:	function(/*nsIAsyncInputStream*/ aStream) {
				try {
					...
					for(;;) {
						...
						_DataLength = 0;
						Log(3, "execute message: \"" + message + "\":" + message.length);
					...
					Log(2, "onInputStreamReady exception. finish");
					Log(2, e.name);
					Log(2, e.message);
					_OnDisconnect();
				}
...
//	methods
	function Done() {
		...
		log('destroy invoked from done method !!');
		_DestroyTransport();
	...
	function _InitTransport(){
		try {
			...
			Log(1, "\t" + e.message);
			log('Invoked Destruction of transport on init');
			_DestroyTransport();
			_StopConnectTimeoutTimer();
			return false;
		}  
